...
3. **RESERVED:** A connection holds this when it plans to modify the database in the future. Only one `RESERVED` lock can exist at a time, but existing `SHARED` locks (readers) can continue reading.
...
4. **Handle `SQLITE_BUSY` Gracefully:** Configure a busy timeout (`PRAGMA busy_timeout = 5000;`) so that SQLite will automatically retry acquiring locks for up to 5000ms before throwing an error to your application code.
